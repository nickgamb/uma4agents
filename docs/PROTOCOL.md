...
| 16 | Several owners of **equal** standing over one resource. A published **mandate** names who is entitled to be counted, at what weight and how many it takes; each owner's authority signs a **verdict** bound to one negotiation and one agreement digest; a **tally** folds the owners' terms into the single document the agent signs, collects verdicts, and issues a grant that carries them. The enforcement point re-verifies every verdict against the keys that owner's authority publishes and re-runs the count from the mandate before allowing the call | UMA 2.0 has exactly one authorization server per protected resource. There is no object for "these parties must both agree", and no way for a relying party to check that they did | An arrangement with no party above the owners cannot put the decision anywhere without privileging the place it is put. Carrying the signed verdicts in the grant makes the counting party *unable to lie* rather than trusted, which is what lets it be an ordinary service rather than a ledger. The four beats and the requesting side are untouched: an unmodified agent negotiates with a tally exactly as with an authorization server. See docs/JOINT.md |
| 17 | An agent holding a connection may **introduce** a sibling: the newcomer arrives with its own key and a short signed statement naming that key, or with its issuer's RFC 8693 `act` claim naming the agent that spawned it. The statement confers nothing — the newcomer skips first contact, then negotiates its own terms under its own key for its own grant. The owner's authority verifies the sponsor against her own active connections, requires a tier she approved in person, refuses an agent that was itself introduced, and requires one operator to have published both keys. Revoking an agent revokes the ones it introduced | UMA 2.0 has no notion of one requesting party standing for another, and no object between "a stranger" and "the same client" | The usual answer is to pass the parent's token down, which makes the workers indistinguishable from the parent in the owner's records, individually unrevocable, and never something she agreed to. Keeping the authority graph one level deep while the task graph nests means every agent is separately visible, separately revocable and separately bounded, and the owner's per-tier rules still apply to each. See docs/SUBAGENTS.md |

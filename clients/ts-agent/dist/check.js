...
if (!process.env.NODE_EXTRA_CA_CERTS)
    console.warn(`NODE_EXTRA_CA_CERTS is unset; expected ${CA}`);
const fetchLab = fetch;
...
function approveInBackground() {
    (async () => {
        const hdrs = await ownerHeaders();
        for (let i = 0; i < 60; i++) {
            const items = (await (await fetchLab(`${AS}/owner/pending`, { headers: hdrs })).json());
...
async function main() {
    ...
    if (!ch)
        return 1;
    check("the challenge carries structured remediation naming the authority and the ticket", ch.remediation?.authorization_server === ch.asUri && ch.remediation?.ticket === ch.ticket);
    console.log("\n== Beat 0: corroborate the authority ==");
    try {
        await corroborate(fetchLab, GATEWAY, ch);
        check("the resource's metadata names the authorization server the challenge did", true);
    }
    catch (e) {
        check("the resource's metadata names the authorization server the challenge did", false, String(e));
        return 1;
    }
    ...
    let rpt, receipt;
    try {
        ({ rpt, receipt } = await runGrant(fetchLab, ch, keys, () => true, { reason: "A second implementation, meeting the first on the wire.", onStatus: say }));
    }
    catch (e) {
        check("a grant is issued", false, String(e));
        return 1;
    }
    ...
    const ok = await mcpCall(fetchLab, GATEWAY, "tools/call", { name: "get_positions", arguments: {} }, signRequest("POST", AUTHORITY, "/mcp", `PoP ${rpt}`, keys));
    ...
    if (again) {
        try {
            await runGrant(fetchLab, again, AgentKeys.loadOrCreate(`${KEYS}/ts-decliner.pem`, "decliner"), () => false, { onStatus: say });
            check("a decline ends the negotiation", false, "granted");
        }
        catch (e) {
            check("a decline ends the negotiation", e instanceof GrantDenied, String(e));
        }
    }
